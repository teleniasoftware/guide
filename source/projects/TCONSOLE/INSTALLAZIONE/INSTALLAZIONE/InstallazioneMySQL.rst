...
.. warning :: Inserire i seguenti parametri **esattamente** come vengono descritti, **senza apporre variazioni** rispetto a quanto specificato: valori diversi di questi parametri potrebbero determinare il fallimento dell'installazione TConsole o anomalie di funzionamento.
...
- lasciare la selezione su **Modify Security Settings** ed impostare la password al valore "**root**"; selezionare la voce **Enable root access from remote machines** e premere **Next**;

.. warning :: la password **deve** essere impostata al valore "root": in caso contrario, le query per la creazione delle tabelle TConsole, eseguite durante la successiva installazione dell'applicativo, falliranno!
